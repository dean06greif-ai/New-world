...
class TestManualTradeGuard:
    def _run(self, doc, source="ki"):
        from types import SimpleNamespace
        mgr = AITradeManager()
        mgr.engine = SimpleNamespace(db=_FakeDb(doc))
        return asyncio.run(
            mgr.apply_action("x", "hold", reason="test", source=source,
                             enforce_limits=False))

    def test_ki_blocked_on_manual_and_external(self):
        for extra in ({"manual_trade": True}, {"external_adopted": True},
                      {"strategy_id": "external"}):
            doc = {"id": "x", "status": "open", "symbol": "QQQUSDT",
                   "side": "SHORT", **extra}
            res = self._run(doc, source="ki")
            assert res["status"] == "blocked"
            assert "Manuell" in res["detail"] or "Extern" in res["detail"]

    def test_manual_source_allowed_on_website_manual_trade(self):
        # Fix 06/26: Website-manuelle Trades (manual_trade ohne external_adopted)
        # dürfen über die Website (source='manual') verwaltet werden.
        doc = {"id": "x", "status": "open", "symbol": "BTCUSDT", "side": "LONG",
               "manual_trade": True, "strategy_id": "external"}
        res = self._run(doc, source="manual")
        assert res == {"status": "ok", "action": "hold"}

    def test_external_adopted_blocked_for_all_sources(self):
        doc = {"id": "x", "status": "open", "symbol": "QQQUSDT", "side": "SHORT",
               "external_adopted": True, "manual_trade": True,
               "strategy_id": "external"}
        for src in ("ki", "manual"):
            res = self._run(doc, source=src)
            assert res["status"] == "blocked"
            assert "Extern" in res["detail"]
